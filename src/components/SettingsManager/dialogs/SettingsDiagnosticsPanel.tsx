  ...
  ({ analysis, isHighlighted }) => {
      ...
      return (
        <div className="relative px-2.5 py-1.5 rounded-md border border-dashed border-border/50 bg-muted/20 opacity-50">
          <div className="flex items-center gap-2">
            ...
            <code className="text-xs text-muted-foreground/60 font-mono break-all">{analysis.path}</code>
            <span className="text-3xs text-muted-foreground/40">{t("settingsManager.analyzer.fileNotFound")}</span>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className={`relative px-2.5 py-1.5 rounded-md border transition-all duration-200 ${isHighlighted ? "border-accent/50 bg-accent/5" : "border-border/50 bg-card hover:border-border"}`}>
        <Badge variant="outline" className={`absolute -top-1.5 right-2 text-4xs px-1 py-0 leading-tight ${SCOPE_COLORS[analysis.scope]}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              <code className="text-xs text-foreground/80 font-mono break-all">{analysis.path}</code>
              {analysis.projectLabel && (
                <Badge variant="secondary" className="text-4xs px-1 py-0 shrink-0">{analysis.projectLabel}</Badge>
              )}
            </div>
            <div className="flex flex-wrap gap-1 mt-1">
              ...
              {analysis.model && (
                <Badge variant="outline" className="text-3xs h-4 px-1 font-mono">{analysis.model}</Badge>
              )}
    ...
    <div className={`px-2.5 py-2 rounded-md border ${SEVERITY_BORDER[issue.severity]}`}>
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0 space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground leading-tight">{t(issue.descriptionKey, issue.descriptionParams ?? {})}</p>
          <div className="flex flex-wrap gap-0.5">
            {issue.affectedScopes.map((scope) => (
              <Badge key={scope} variant="outline" className={`text-4xs px-1 py-0 ${SCOPE_COLORS[scope] ?? ""}`}>
                {t(`settingsManager.analyzer.scope.${scope}`)}
              </Badge>
            ))}
          </div>
          <p className="text-3xs italic text-muted-foreground/70 leading-tight">{t(issue.recommendationKey)}</p>
        </div>
      </div>
    </div>
  );
    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex items-center gap-1.5 w-full px-2 py-1.5 rounded-md hover:bg-muted/50 transition-colors text-left">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {errors > 0 && (
            <Badge variant="secondary" className="text-3xs h-3.5 px-1 bg-destructive/10 text-destructive">
              {errors}
            </Badge>
          )}
          {warnings > 0 && (
            <Badge variant="secondary" className="text-3xs h-3.5 px-1 bg-amber-500/10 text-amber-600">
              {warnings}
            </Badge>
          )}
          {infos > 0 && (
            <Badge variant="secondary" className="text-3xs h-3.5 px-1 bg-blue-500/10 text-blue-600">
              {infos}
            </Badge>
          )}
        </div>
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 border-b border-border/50 bg-muted/20 shrink-0">
      <div className="flex items-center gap-2 flex-1">
        ...
        <div className="flex items-center gap-1.5">
          {errors > 0 && (
            <Badge variant="secondary" className="text-xs h-5 bg-destructive/10 text-destructive">
              {t("settingsManager.diagnostics.errorCount", { count: errors })}
            </Badge>
          )}
          {warnings > 0 && (
            <Badge variant="secondary" className="text-xs h-5 bg-amber-500/10 text-amber-600">
              {t("settingsManager.diagnostics.warningCount", { count: warnings })}
            </Badge>
          )}
          {infos > 0 && (
            <Badge variant="secondary" className="text-xs h-5 bg-blue-500/10 text-blue-600">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
        <TabsList className="w-full justify-start rounded-none border-b border-border/50 bg-transparent h-auto p-0 shrink-0">
          ...
          >
            ...
            {issueCounts.total > 0 && (
              <Badge variant="secondary" className={`ml-2 text-xs h-4 px-1.5 ${issueCounts.errors > 0 ? "bg-destructive/10 text-destructive" : "bg-amber-500/10 text-amber-600"}`}>
                {issueCounts.total}
              </Badge>
            )}
          </TabsTrigger>
        ...
        <TabsContent value="files" className="flex-1 overflow-auto m-0 p-4 space-y-5">
          ...
          <div className="border-t border-border/50 pt-4 space-y-2">
            ...
            <p className="text-xs text-muted-foreground text-center">{t("settingsManager.analyzer.backupHint")}</p>
          </div>
        </TabsContent>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-card border border-border/50 rounded-lg p-4 font-mono text-xs space-y-1">
              ...
              <div className="pl-4 flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-3xs h-4">{t("settingsManager.analyzer.recommended.globalSettings")}</Badge>
              </div>
              <div className="text-muted-foreground mt-3">
                <span className="text-emerald-500">~/.claude.json</span>
                <Badge variant="outline" className="text-3xs h-4 ml-2">{t("settingsManager.analyzer.recommended.globalMcp")}</Badge>
              </div>
              <div className="text-muted-foreground mt-3"><span className="text-blue-500">your-project/</span></div>
              <div className="pl-4 flex items-center gap-2">
                <ChevronRight aria-hidden className="w-3 h-3 text-muted-foreground/50" />
                <span>.mcp.json</span>
                <Badge variant="outline" className="text-3xs h-4">{t("settingsManager.analyzer.recommended.projectMcp")}</Badge>
              </div>
              <div className="pl-4 flex items-center gap-2">
                <ChevronRight aria-hidden className="w-3 h-3 text-muted-foreground/50" />
                <span>.claude/settings.json</span>
                <Badge variant="outline" className="text-3xs h-4">{t("settingsManager.analyzer.recommended.teamSettings")}</Badge>
              </div>
              <div className="pl-4 flex items-center gap-2 text-muted-foreground/60">
                ...
                <Badge variant="outline" className="text-3xs h-4 opacity-50">gitignore</Badge>
              </div>
            </div>
          </div>
