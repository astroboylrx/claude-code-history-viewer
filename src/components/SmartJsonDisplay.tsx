    ...
    if (Array.isArray(value)) {
        return (
            <div className="flex flex-wrap gap-1 mt-0.5">
                {value.map((v, i) => (
                    <span key={i} className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-muted/50 border border-border/50 text-xs text-foreground font-mono">
                        {String(v)}
                    </span>
                ))}
            </div>
                ...
                if (['description', 'prompt', 'task'].includes(key) && typeof value === 'string') {
                    return (
                        <div key={key} className="flex flex-col gap-1 pb-1 border-b border-border/10 last:border-0">
                            <div className="flex items-center gap-1.5 text-muted-foreground uppercase text-xs font-bold tracking-wider opacity-70">
                                {icon}
