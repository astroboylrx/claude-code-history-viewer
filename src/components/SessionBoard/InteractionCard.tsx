...
const FileEditDisplay = ({ toolUseBlock }: { toolUseBlock: ReturnType<typeof getToolUseBlock> }) => {
    ...
    if (path && typeof path === 'string') {
        ...
        return (
            <div className="flex items-center gap-1.5 px-2 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded text-xs text-emerald-600 font-medium mb-1">
...
const ExitCodeDisplay = ({ message }: { message: ClaudeMessage }) => {
    ...
    return (
        <div className={clsx("flex items-center gap-1 text-3xs font-mono px-1.5 py-0.5 rounded border self-start",
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-1.5 text-xs font-medium">
                            ...
                            <div className="flex flex-col">
                                <span className="uppercase text-xs font-bold tracking-wide opacity-80">
                        ...
                        <div className="text-xs leading-tight text-foreground/90 font-mono line-clamp-4 whitespace-pre-wrap break-words border-t border-border/20 pt-1 mt-0.5">
                            {totalMessagesCount > 1 ? (
                                <div className="flex flex-col gap-1">
            ...
            <>
                ...
                >
                    ...
                    {agentName !== generalPurpose && (
                        <div className="text-4xs font-bold uppercase tracking-widest text-muted-foreground/40 leading-none select-none">
                            {agentName}
                        </div>
                    )}
                    ...
                    <div className="flex gap-2 items-start w-full">
                        ...
                        <div className="flex-1 min-w-0">
                            {toolUseBlock && (
                                <div
                                    className="text-3xs font-medium uppercase tracking-tight text-accent opacity-90 mb-0.5 flex items-center gap-1.5 hover:opacity-100 cursor-pointer"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        onToggleSticky?.();
                                    }}
                                >
                                    ...
                                    {siblings && siblings.length > 0 && (
                                        <span className="flex items-center gap-0.5 text-4xs bg-accent/10 text-accent px-1 rounded-sm border border-accent/20">
                            ...
                            {isShell && shellCommand && (
                                <p className="text-xs font-mono text-[var(--tool-terminal)] truncate opacity-70 mb-0.5">
    ...
    return (
        <>
            ...
            >
                {editedMdFile ? (
                    <div
                        className="flex items-center gap-1.5 px-2 py-1 bg-amber-500/10 border border-amber-500/20 rounded text-xs text-amber-600 font-medium mb-1 cursor-help group/md"
                    ...
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-[var(--tool-terminal)]/10 border border-[var(--tool-terminal)]/20 rounded text-xs text-[var(--tool-terminal)] font-medium mb-1">
                ...
                <div className="flex justify-between items-center border-b border-border/10 pb-1 mb-0.5">
                    <div className="flex items-center gap-2">
                        ...
                        {isCommit && <span className="text-3xs bg-indigo-500/10 text-indigo-600 px-1 rounded border border-indigo-200 uppercase tracking-wider font-bold cursor-pointer">GIT</span>}
                        {isShell && <span className="text-3xs bg-[var(--tool-terminal)]/10 text-[var(--tool-terminal)] px-1 rounded border border-[var(--tool-terminal)]/20 uppercase tracking-wider font-bold cursor-pointer">SHELL</span>}
                        {editedMdFile && <span className="text-3xs bg-amber-500/10 text-amber-600 px-1 rounded border border-amber-200 uppercase tracking-wider font-bold cursor-pointer">DOCS</span>}
                        ...
                        {toolFrequency && (
                            <div className="flex items-center gap-1.5 ml-1">
                                {(Object.entries(toolFrequency) as [string, number][]).sort((a, b) => b[1] - a[1]).map(([name, count]) => (
                                    <div
                                        key={name}
                                        className="flex items-center gap-0.5 text-3xs text-muted-foreground/80 bg-muted/30 px-1 rounded-sm border border-border/20 hover:bg-muted/50 cursor-pointer transition-colors"
                                        title={`${count}x ${name}`}
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            onToggleSticky?.();
                                    ...
                                    >
                                        ...
                                        {count > 1 && <span className="opacity-50 text-4xs ml-px">({count})</span>}
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        {isCancelled && (
                            <span className="text-3xs uppercase font-bold text-orange-500 tracking-wide border border-orange-500/30 px-1 rounded">{t("session.interaction.cancelled")}</span>
                        )}
                    </div>
                ...
                {(message.type === 'assistant' && message.usage) && (
                    <div className="mt-auto pt-1 flex gap-2 text-3xs text-muted-foreground opacity-60 font-mono items-center">
                ...
                <div className="flex gap-2 mt-1">
                    ...
                    {isClaudeAssistantMessage(message) && message.stop_reason === 'max_tokens' && (
                        <div className="flex items-center gap-1 text-3xs font-mono px-1.5 py-0.5 rounded border self-start text-orange-600 bg-orange-500/5 border-orange-500/20" title={t("session.interaction.cutoffTitle")}>
                ...
                {isError && (
                    <div className="mt-1 p-1 bg-destructive/10 rounded text-3xs text-destructive border border-destructive/20 font-mono italic flex items-center gap-1">
