            ...
            <div className={clsx(
                ...
                {zoomLevel === 0 ? (
                    <div className="flex flex-col items-center gap-1.5 text-center h-full justify-between">
                        ...
                        <div className="mt-auto text-4xs font-mono text-muted-foreground">
                    ...
                    <div className="flex flex-col h-full gap-2">
                            ...
                            <div className="flex gap-3 items-center">
                                ...
                                {stats.commitCount > 0 && (
                                    <div className="flex items-center gap-1 text-indigo-500" title={t("session.board.gitCommits")}>
                                        ...
                                        <span className="text-xs font-bold">{stats.commitCount}</span>
                                    </div>
                                )}

                                <div className="flex items-center gap-1 text-emerald-500" title={t("analytics.inputTokens")}>
                                    <TrendingUp className="w-3 h-3" />
                                    <span className="text-xs font-mono">{formatNumber(stats.inputTokens || 0)}</span>
                                    <span className="text-3xs opacity-60">{t("analytics.in", "in")}</span>
                                </div>

                                <div className="flex items-center gap-1 text-purple-500" title={t("analytics.outputTokens")}>
                                    <Zap className="w-3 h-3" />
                                    <span className="text-xs font-mono">{formatNumber(stats.outputTokens || 0)}</span>
                                    <span className="text-3xs opacity-60">{t("analytics.out", "out")}</span>
                                </div>
                        ...
                        <div className="flex items-baseline gap-2 pb-1 border-b border-border/20">
                            <div className="text-xl font-bold font-mono text-foreground">
                                ...
                                <span className="text-xs text-muted-foreground font-normal ml-1">{t("analytics.tokens")}</span>
                            </div>
                        ...
                        <div className="flex flex-col gap-1.5 py-1">
                            ...
                            <div className="flex items-center gap-2 pt-1 border-t border-border/10">
                                ...
                                {stats.shellCount > 0 && (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-bold font-mono">{stats.shellCount}</span>
                                    </button>
                                )}
                                    ...
                                    if (createdCount > 0) {
                                        return (
                                            ...
                                            >
                                                ...
                                                <span className="text-xs font-bold font-mono">{createdCount}</span>
                                            </button>
                                        );
                                        ...
                                        return (
                                            ...
                                            >
                                                <FilePlus className="w-3 h-3" />
                                                <span className="text-xs font-bold font-mono">{stats.fileToolCount}</span>
                                            </button>
                                        );
                                    }
                                    return null;
                                ...
                                {stats.searchCount > 0 && (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-bold font-mono">{stats.searchCount}</span>
                                    </button>
                                )}
                                ...
                                {stats.webCount > 0 && (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-bold font-mono">{stats.webCount}</span>
                                    </button>
                                )}
                                ...
                                {stats.mcpCount > 0 && (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-bold font-mono">{stats.mcpCount}</span>
                                    </button>
                                )}
                                ...
                                {stats.gitToolCount > 0 && (
                                    ...
                                    >
                                        <GitCommit className="w-3 h-3" />
                                        <span className="text-xs font-bold font-mono">{stats.gitToolCount}</span>
                                    </button>
                                )}
                                ...
                                {stats.hasMarkdownEdits && (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-bold font-mono">{stats.markdownEditCount}</span>
                                    </button>
                                )}
                                ...
                                {stats.fileEditCount > 0 && (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-bold font-mono">{stats.fileEditCount}</span>
                                    </button>
                                )}
                                ...
                                {stats.codeReadCount > 0 && (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-bold font-mono">{stats.codeReadCount}</span>
                                    </button>
                                )}
                            </div>
                            ...
                            {data.gitCommits && data.gitCommits.length > 0 && (
                                <div className="flex flex-wrap gap-x-2 gap-y-1 mt-1 pt-1 border-t border-border/10">
                                    <div className="text-4xs uppercase font-bold text-blue-500/70 tracking-tighter w-full">{t("session.board.gitCommits")}</div>
                                    {data.gitCommits.slice(0, 2).map(commit => (
                                        <div key={commit.hash} className="flex items-center gap-1.5 text-3xs text-blue-600/80 font-mono bg-blue-500/5 px-1.5 py-0.5 rounded border border-blue-500/10 max-w-full overflow-hidden" title={commit.message}>
                                            <GitCommit className="w-2.5 h-2.5 shrink-0" />
                                            <span className="truncate">{commit.message}</span>
                                            <code className="text-4xs opacity-40 shrink-0">{commit.hash.substring(0, 7)}</code>
                                        </div>
                                    ))}
                                    {data.gitCommits.length > 2 && (
                                        <span className="text-3xs text-muted-foreground/40 self-center">{t("session.board.moreItems", { count: data.gitCommits.length - 2 })}</span>
                                    )}
                                </div>
                            )}
                        </div>
